...
func show_cutscene(cutscene_definition: Dictionary) -> void:
	...
	if _cutscene_player_sprite.texture == null:
		_cutscene_player_sprite.texture = load("res://sprites/dialogue_player.png") as Texture2D
	if _cutscene_player_sprite.texture == null:
...
func _populate_cutscene_background_features(cutscene_level: int) -> void:
	...
	var boardwalk_fill_texture: Texture2D = boardwalk_texture
	var boardwalk_lane_texture: Texture2D = boardwalk_plank_texture if boardwalk_plank_texture != null else boardwalk_texture
	...
	for placement_raw in placements:
		...
		if is_boardwalk:
			if boardwalk_fill_texture == null:
				continue
			var fill := TextureRect.new()
			fill.texture = boardwalk_fill_texture
