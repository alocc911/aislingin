...
func _is_enemy_boss_home_destination(province_id: int) -> bool:
	...
	if not _is_active_boss_home_destination(province_id):
		return false
	# Boss-home identity should be driven by the active boss roster, not by mutable
	# province ownership fields. During deferred friendly-boss invasions we temporarily
	# stamp faction_id to the friendly-boss faction while the province remains an enemy
	# boss home; relying on faction/type here misclassifies the province and breaks
	# subsequent boss-home targeting and diagnostics.
	return not _is_friendly_boss_home_destination(province_id)
...
func resolve_march_arrival(destination_id: int, moving_troops: int, source_type: String = LevelConfig.PROVINCE_TYPE_ENEMY, source_faction: int = LevelConfig.ENEMY_FACTION_DEFAULT, source_id: int = -1) -> bool:
	...
	_trigger_auto_engagement_preview(destination_id, moving_troops, destination_troops_before, source_faction, destination_owner_faction_before, destination_buildings_before, source_type, destination_type)
...
func _clear_friendly_boss_pending_fields(province_state: Dictionary) -> void:
...
func _is_valid_friendly_boss_pending_invasion(province_state: Dictionary, province_id: int, expected_boss_id: int = -1) -> bool:
	...
	if expected_boss_id >= 0 and invading_boss_id != expected_boss_id:
		return false
	if invading_boss_id < 0:
		return false
	if maxi(0, int(province_state.get("friendly_boss_invading_troops", 0))) <= 0:
		return false
	if int(province_state.get("friendly_boss_invasion_started_turn", -1)) < 0:
		return false
	...
	if boss_system == null:
		return false
	if boss_system.has_method("is_boss_active") and not bool(boss_system.is_boss_active(invading_boss_id)):
		return false
	if boss_system.has_method("get_boss_current_province_id") and int(boss_system.get_boss_current_province_id(invading_boss_id)) != province_id:
		return false
	var defending_enemy_boss_id: int = int(province_state.get("friendly_boss_defending_enemy_boss_id", -1))
	if defending_enemy_boss_id >= 0 and boss_system.has_method("is_boss_active") and not bool(boss_system.is_boss_active(defending_enemy_boss_id)):
		return false
	return true


func normalize_friendly_boss_invasion_state(log_context: String = "") -> int:
	if _main == null:
		return 0
	...
	for province_state_any in _main._province_persistence:
		...
		if bool(province_state.get("friendly_boss_invasion_pending", false)):
			if _is_valid_friendly_boss_pending_invasion(province_state, province_id):
				continue
			_clear_friendly_boss_pending_fields(province_state)
			cleared_count += 1
			continue
		var has_orphan_invader_fields: bool = int(province_state.get("friendly_boss_invading_troops", 0)) > 0 or int(province_state.get("friendly_boss_invader_id", -1)) >= 0 or int(province_state.get("friendly_boss_defending_enemy_boss_id", -1)) >= 0 or int(province_state.get("friendly_boss_invasion_started_turn", -1)) >= 0
		if has_orphan_invader_fields:
			_clear_friendly_boss_pending_fields(province_state)
			cleared_count += 1
	if cleared_count > 0 and log_context.strip_edges() != "":
		_append_automated_engagement_log_with_priority("Friendly boss invasion cleanup: cleared %d stale pending marker%s (%s)." % [
			cleared_count,
			"" if cleared_count == 1 else "s",
			log_context.strip_edges()
		], 98)
...
func _move_friendly_boss_after_marches() -> void:
	if _main == null:
		return
	normalize_friendly_boss_invasion_state("before friendly boss movement")
	if _main.province_system == null:
	...
	if bool(src_state.get("friendly_boss_invasion_pending", false)) and int(src_state.get("friendly_boss_invader_id", -1)) == friendly_boss_id:
		...
		if _is_valid_friendly_boss_pending_invasion(src_state, source_id, friendly_boss_id):
			_append_automated_engagement_log_with_priority("Friendly boss move debug: waiting at %s because a boss-home invasion is still pending (started_turn=%d pending_troops=%d)." % [
				_format_province_label(source_id),
				pending_started_turn,
				pending_invading_troops
			], 98)
			return
		_clear_friendly_boss_pending_fields(src_state)
		_append_automated_engagement_log_with_priority("Friendly boss move debug: cleared stale pending-invasion marker at %s (started_turn=%d pending_troops=%d) before moving." % [
			_format_province_label(source_id),
			pending_started_turn,
			pending_invading_troops
		], 98)
...
func _trigger_auto_engagement_preview(province_id: int, attacker_troops: int, defender_troops: int, attacker_faction_id: int, defender_faction_id: int, defender_buildings: int = 0, attacker_type: String = LevelConfig.PROVINCE_TYPE_ENEMY, defender_type: String = LevelConfig.PROVINCE_TYPE_NEUTRAL) -> void:
	if _main == null or not _main.has_method("render_auto_engagement_preview"):
		return
	if attacker_troops <= 0 and defender_troops <= 0:
		return
	_main.render_auto_engagement_preview(province_id, attacker_troops, defender_troops, attacker_faction_id, defender_faction_id, defender_buildings, attacker_type, defender_type)
...
func apply_invasion_building_damage_and_conquest(province_state: Dictionary) -> void:
	...
	_trigger_auto_engagement_preview(province_id, invading_troops, defenders_before, attacking_faction, owner_faction_before, int(province_state.get("remaining_buildings", 0)), LevelConfig.PROVINCE_TYPE_ENEMY, province_type_before)
...
func apply_undefended_invasion_damage(skip_province_id: int = -1, eligible_province_ids: Array[int] = [], restrict_to_eligible: bool = false) -> void:
	...
	normalize_friendly_boss_invasion_state("after pending resolution")
	resolve_destroyed_enemy_provinces()
	if _main.province_system != null:
		_apply_province_visuals_if_preview_idle()
...
func _resolve_pending_friendly_boss_invasions(skip_province_id: int, eligible_lookup: Dictionary, restrict_to_eligible: bool) -> void:
	...
	for province_state in _main._province_persistence:
		...
		if defending_enemy_boss_id >= 0 and boss_system.has_method("get_boss_home_troop_count"):
			...
			# Resolve losses using the actual defending boss-home troops so combat stops once
			# either side runs out.
			defending_troops = maxi(0, int(boss_system.get_boss_home_troop_count(defending_enemy_boss_id)))
		...
		var pending_continues: bool = surviving_invaders > 0 and surviving_defenders > 0
		province_state["remaining_troops"] = surviving_defenders
		province_state["friendly_boss_invasion_pending"] = pending_continues
		province_state["friendly_boss_invading_troops"] = surviving_invaders if pending_continues else 0
		province_state["friendly_boss_invader_id"] = invading_boss_id if pending_continues else -1
		province_state["friendly_boss_defending_enemy_boss_id"] = defending_enemy_boss_id if pending_continues else -1
		province_state["friendly_boss_invasion_started_turn"] = current_turn if pending_continues else -1
		...
		if surviving_invaders <= 0 and invader_was_present and mutual_losses > 0 and invading_boss_id >= 0:
			var verified_invader_troops: int = 0
			if boss_system.has_method("get_boss_home_troop_count"):
				verified_invader_troops = maxi(0, int(boss_system.get_boss_home_troop_count(invading_boss_id)))
			if verified_invader_troops > 0:
				# Defensive guardrail: if subsystem rounding/order effects report surviving
				# invader boss troops, keep the boss alive and preserve those troops.
				surviving_invaders = verified_invader_troops
				pending_continues = surviving_defenders > 0
				province_state["friendly_boss_invasion_pending"] = pending_continues
				province_state["friendly_boss_invading_troops"] = surviving_invaders if pending_continues else 0
				province_state["friendly_boss_invader_id"] = invading_boss_id if pending_continues else -1
				province_state["friendly_boss_defending_enemy_boss_id"] = defending_enemy_boss_id if pending_continues else -1
				province_state["friendly_boss_invasion_started_turn"] = current_turn if pending_continues else -1
			elif boss_system.has_method("mark_boss_dead"):
...
func advance_grand_map_turn_after_rest(status_text: String, lock_province_id: int = -1) -> void:
	advance_turn_and_run_automation(1, status_text, lock_province_id)


func advance_turn_and_run_automation(turns_to_advance: int, status_context: String, lock_province_id: int = -1, initial_skip_province_id: int = -1, initial_eligible_province_ids: Array[int] = []) -> void:
	if _main == null:
		return
	...
	for turn_step in range(turns_count):
		_main.level_index += 1
		_main.turn_number += 1
		var preexisting_invaded_province_ids: Array[int] = eligible_province_ids.duplicate()
		if preexisting_invaded_province_ids.is_empty():
			preexisting_invaded_province_ids = get_invaded_friendly_province_ids()
		run_enemy_turn_cycles(1, skip_province_id, preexisting_invaded_province_ids)
		skip_province_id = -1
		eligible_province_ids = get_invaded_friendly_province_ids()
	...
	if _main.ui_bridge != null:
		_main.ui_bridge.ui_set_status(build_automated_engagement_status_text(status_context))
